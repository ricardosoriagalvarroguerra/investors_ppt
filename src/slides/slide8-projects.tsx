...
function idToISO3(id: number | string): string | null {
...
function Slide8({ active }) {
  ...
  return (
    <div className="s8">
      <header className="s8__head">
        <div className="s8__h-num">08 · Portfolio</div>
        <h2 className="s8__h-title">Projects' <span className="thin">Snapshot</span></h2>
        <div className="s8__h-meta">5 representative operations · 4 member countries</div>
      </header>

      <div className="s8__body">
        <aside className="s8__list">
          <div className="s8__list-eye">Operations</div>
          {PROJECTS.map((pr, i) => (
            <button
              key={pr.id}
              className={"s8__row " + (i === sel ? "is-active" : "")}
              onMouseEnter={() => setSel(i)}
              onClick={() => setSel(i)}
            >
              <span className="s8__row-n">{String(i + 1).padStart(2, "0")}</span>
              <span className="s8__row-body">
                <span className="s8__row-title">{pr.title}</span>
                <span className="s8__row-meta">{pr.iso3} &middot; USD {pr.contribution}M</span>
              </span>
              <span className="s8__row-arrow">→</span>
            </button>
          ))}
        </aside>

        <div className="s8__map" ref={wrapRef}>
          <svg ref={svgRef} viewBox={`0 0 ${size.w} ${size.h}`} preserveAspectRatio="xMidYMid meet" />
          <div className="s8__map-eye">Click a pin or list item to inspect</div>
        </div>

        <aside className="s8__detail">
          <div className="s8__detail-eye">Project · {p.iso3}</div>
          <h3 className="s8__detail-title">{p.title}</h3>

          <div className="s8__metrics">
            <div className="s8__metric">
              <div className="s8__metric-num">
                <CountUp to={p.contribution} duration={900} play={active} />
                <span className="s8__metric-unit">M</span>
              </div>
              <div className="s8__metric-lab">FONPLATA contribution · USD</div>
            </div>
            <div className="s8__metric">
              <div className="s8__metric-num">
                {fmtBen(p.beneficiaries)}
              </div>
              <div className="s8__metric-lab">Direct {p.benUnit}</div>
            </div>
          </div>

          <div className="s8__sdg-eye">SDGs addressed</div>
          <div className="s8__sdg-grid">
            {p.sdgs.map((n, i) => (
              <div className="s8__sdg" key={n}>
                <div className="s8__sdg-num">{n}</div>
                <div className="s8__sdg-label">{p.sdgLabels[i]}</div>
              </div>
            ))}
          </div>

          <div className="s8__detail-foot">
            <span>Sovereign-guaranteed</span>
